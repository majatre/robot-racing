...
START_POSE = np.array([-2.5, -2.5, 0.], dtype=np.float32)
MAX_ITERATIONS = 500


def sample_random_position(occupancy_grid):
  # Sample a valid random position using uniform distribution.
...
def rrt(start_pose, goal_position, occupancy_grid):
  ...
  for _ in range(MAX_ITERATIONS): 
    ...
    u = None
    for n, cost in potential_parent:
      d =  np.linalg.norm(position - n.position)
      if d > .2 and d < 4 and n.direction.dot(position - n.position) / d > 0.70710678118:
        u = n
        break
    else:
      continue
...
if __name__ == '__main__':
  ...
  #   start = time.time()
  #   start_node, final_node = rrt(START_POSE, GOAL_POSITION, occupancy_grid)
  #   path_lengths.append(get_path_length(get_path(final_node)))
  #   end = time.time()
