...
def get_relative_position(absolute_pose, absolute_position):
  ...
  rotation = np.array([[np.cos(absolute_pose[2]), np.sin(absolute_pose[2])], 
                       [-np.sin(absolute_pose[2]), np.cos(absolute_pose[2])]]) 
  relative_position = np.dot(rotation, absolute_position)
  relative_position -= absolute_pose[0:2]
