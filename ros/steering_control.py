...
def calculate_error(pose, path_points):
    ...
    p4 = sum(
        path_points[min_point + lookahead:min_point + lookahead + 3]) / len(
        path_points[min_point + lookahead:min_point + lookahead + 3])
    ...
    if len(path_points) <= min_point + 3:
        ...
        curvature = sum(
            [sum(curvatures) / len(curvatures),
             sum(curvatures[:int(lookahead / 2)]) / len(
                 curvatures[:int(lookahead / 2)]),
             sum(curvatures[:int(lookahead / 4)]) / len(
                 curvatures[:int(lookahead / 4)])]
        ) / 3

        direction = path_points[min_point + 1]
        factor = max_velocity
